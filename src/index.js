...
export function enhancedImages(options) {
	...
	const imagetools_instance = imagetools_plugin(dynamic_enabled);
	const public_imagetools_instance = dynamic_enabled
		? preserve_public_exclusion(imagetools_instance)
		: imagetools_instance;
	return !process.versions.webcontainer
		? [
				image_plugin(public_imagetools_instance, normalized, imagetools_instance),
				public_imagetools_instance
			]
		: [];
}
...
function imagetools_plugin(allow_public = false) {
	...
	const imagetools_opts = {
		// Dynamic publicDir files are an explicit opt-in and are loaded through a
		// private virtual module rather than through Vite's public-file import path.
		...(allow_public && {
			exclude: undefined,
