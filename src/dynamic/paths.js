 ...
 * a Vite-root key such as `/src/routes/+page.svelte`, never an absolute path.
 *
 * @param {string} value
...
/**
 * Create the root-relative component key embedded in generated Svelte code.
 *
 * @param {string} filename
 * @param {string} root
 */
export function component_runtime_key(filename, root) {
	...
	if (relative === null || relative === '') {
		throw new Error(
			`@itznotabug/enhanced-img: component is outside the Vite root: ${JSON.stringify(filename)}`
		);
	}
...
export function serialize_aliases(aliases, options) {
	...
	for (const entry of entries) {
		if (entry.customResolver !== undefined && entry.customResolver !== null) {
			options.warn?.(
				`@itznotabug/enhanced-img dynamic: alias ${format_alias_find(entry.find)} uses a custom resolver and cannot be used in runtime image paths`
			);
			continue;
		}
