/**
 * A deliberately small abstract evaluator for ESTree expressions. It only
 * works with values that it created itself and never calls application code
 * (including getters or coercion methods).
 *
 * Contexts are opaque, immutable handles. A markup walker can extend a context
 * when it enters an iteration or encounters a lexical declaration.
...
/**
 * Create the root evaluation context from an ESTree Program.
...
/**
 * Add an ESTree const declaration to a context. The returned context is
 * independent of the input context and is intended for subsequent siblings in
 ...
 * @param {{ cap?: number }} [options]
 */
export function extend_const_context(context, declaration, options = {}) {
...
/**
 * Extend a context for an iteration. A statically finite array creates one
 * environment per row. If the iterable is opaque, its bindings are installed
 * as Unknown so consumers can continue conservative symbolic analysis.
 *
 * The expansion guard is checked while rows are streamed; no Cartesian array
 * is allocated before the cap is enforced.
 *
 * @param {object} context
 * @param {{ iterable: any, pattern: any, index?: string | null }} iteration
 * @param {{ cap?: number }} [options]
 */
export function extend_iteration_context(context, iteration, options = {}) {
	const state = get_context_state(context);
	const cap = normalize_cap(options.cap ?? state.cap);
	/** @type {Environment[]} */
	const envs = [];
	...
	for (let parent_index = 0; parent_index < state.envs.length; parent_index += 1) {
		...
		const iterable = evaluate_in_env(iteration?.iterable, parent_env, cap);

		if (iterable.kind === 'overflow') {
			context_overflow = iterable;
			envs.push(bind_unknown_pattern(parent_env, iteration?.pattern, iteration?.index));
			continue;
		}
		...
		for (const value of iterable.values) {
			...
			for (let index = 0; index < value.length; index += 1) {
				...
				let row_env =
					value[index] === ABSENT
						? bind_unknown_pattern(parent_env, iteration?.pattern, iteration?.index)
						: bind_pattern_eager(
								parent_env,
								iteration?.pattern,
				...
				if (typeof iteration?.index === 'string') {
					row_env = add_value_binding(row_env, iteration.index, {
						kind: 'finite',
						values: [index]
					});
				}
