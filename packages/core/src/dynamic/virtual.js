 ...
 * `register_profile` while transforming framework modules.
 *
...
export class DynamicVirtualModules {
	...
	constructor(options) {
		...
		this.plugin = {
			name: 'vite-plugin-emage-dynamic-modules',
			resolveId(id) {
	...
	/**
	 * Replace the exact profile set imported by one transformed module and
	 * retire managed profiles that no module imports anymore.
	 *
	 * @param {string} owner
	...
	set_owner_profiles(owner, profile_hashes) {
		...
		for (const hash of next) {
			if (!this.profiles.has(hash)) {
				throw virtual_error(`module ${owner} references an unknown profile ${hash}`);
			}
...
function virtual_error(message) {
	return new Error(`@itznotabug/emage-core: ${message}`);
}
