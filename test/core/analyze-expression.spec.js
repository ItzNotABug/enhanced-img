...
describe('finite expression evaluation', () => {
	...
	it('preserves correlations between values from the same finite each row', () => {
		...
		const context = extend_iteration_context(root, iteration(each));
	...
	it('extends the lexical context after an ESTree const declaration', () => {
		const { ast, expression, each } = fixture(`
			...
			{/each}
		`);
		const each_context = extend_iteration_context(
			create_evaluation_context(ast.instance.content),
			iteration(each)
		);
		const const_tag = each.body.nodes.find((node) => node.type === 'ConstTag');
		const context = extend_const_context(each_context, const_tag.declaration);
